...
npx tsx scripts/publish-update.ts --old-hashes-dir <dir> --out <dir>   # multi-hop: one patch per snapshot in <dir>
```
...
**Multi-hop patches** — A client only gets a patch if `manifest.db.patches[<its DB version>]` exists. A single patch (from the immediately-previous publish) means any app more than one publish behind — including every native store build, whose bundled DB is usually several publishes old — falls back to a full download. So `publish-update.ts` accepts `--old-hashes-dir <dir>`: it loads every `*.json` hash snapshot in the dir and generates a fresh **direct-to-current** patch from each (a client applies exactly one patch regardless of how far back it is). All target current, so `mergeManifestPatches` keeps them up to `--keep-patches`. `buildPatchEntry()` does per-snapshot generate+gzip+size-check — **>50% rows changed, or patch >50% of DB size → skipped** (that version falls back to full download). This guard auto-caps far-back versions, so a generous hop count is safe. `publish-data.yml` sets the count via the job-level `KEEP_PATCHES` env (currently **15** — tune in one place), builds the keep-set as `[prev current_version, ...existing patch keys]` (trimmed to `KEEP_PATCHES`), downloads each version's snapshot, and runs publish-update against the dir. Each publish archives its snapshot to **both** `db-hashes.json` (latest mirror, plain) and `hashes/<version>.json.gz` (gzipped per-version archive, ~2.5 MB), so a later run can patch FROM that version. It also **backfills** a gzipped archive for every keep-set source it just used — without this, a version only reachable via `db-hashes.json` while current (e.g. the deployed production DB, never per-version-archived) would fall off after a single publish; the backfill keeps it patch-able for the full window. Only versions with no recoverable snapshot anywhere (published before multi-hop and already superseded) warn and fall back to full download.

**Client apply is single-shot and atomic** — `applyUpdate()` (db.ts) downloads exactly ONE patch (the client's version → current, regardless of how many publishes behind) and applies it transactionally: web via worker `BEGIN/COMMIT/ROLLBACK`, native via `execute({ transaction: true })` (Android `beginTransaction` + `setTransactionSuccessful` only on success, `endTransaction` in `finally`). There is no multi-patch sequence to partially apply. A truncated/half-downloaded patch fails to gunzip → `applyUpdate` throws → `cacheVersionWrite` never runs, so the client stays on its old version and retries. Patches are `.sql.gz`; the client decodes via `DecompressionStream("gzip")` or an `fflate` fallback (iOS < 16.4). The native split parser (`SqliteDb.java splitStatements`) is quote-aware so semicolons inside JSON `data` literals don't break statement splitting.

**Publish idempotency guard** — `publish-data.yml`'s `Check whether DB content changed` step compares the freshly-built DB version against R2's manifest `current_version`; if equal, all publish/upload steps are skipped (`if: steps.guard.outputs.changed == 'true'`). The DB hash is content-deterministic, so "version unchanged" exactly means "data unchanged" — a push touching `data/**` without altering built content (e.g. stripped `_meta` fields, whitespace) won't republish or churn the manifest.

**Native bundle preflight** (`scripts/check-bundle-db-published.ts`) — wired into `npm run build:mobile` between `build` and `cap sync`. Compares local `data/db-version.txt` against R2 `current_version` and prints a loud warning when the about-to-be-bundled DB is not a published version (orphan off the patch chain → every install gets a full download). Warn-only by default (offline builds, intentional unpublished-data testing); `LEXIKLAR_REQUIRE_PUBLISHED_DB=1` makes it a hard error. **Operational rule: publish data first, then build the native release from the same commit**, so the bundle ships the published DB and stays patchable for the next `KEEP_PATCHES` publishes.
