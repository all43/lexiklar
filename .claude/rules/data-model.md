...
- **text_linked**: NOT stored in shard files — derived at build time from `annotations` by the resolver in `scripts/lib/text-linked.ts` (`annotateExampleText`). Written only to the SQLite DB blob. The resolver maintains a per-form cursor so annotations on a recurring form (e.g. *Mann ... am Mann*) each link to their own occurrence instead of collapsing onto the first one
