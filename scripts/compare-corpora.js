/**
 * compare-corpora.js — Corpus coverage comparison tool
 *
...
const WIKI_SIZE         = flagVal("--download-wikipedia", "300K");
const CUSTOM_WORDS_FILE = flagVal("--words", null);
const FRESH             = flag("--fresh");

// Rank cutoffs to test
const CUTOFFS = [3000, 5000, 10000, 20000, 50000, 100000];
...
function loadCorpus(def) {
  ...
  if (def.format === "leipzig") {
    ...
    const entries = rows.slice(1)
      .filter(r => typeof r[0] === "string" && typeof r[4] === "number" && r[4] > 0)
      .sort((a, b) => b[4] - a[4]);
    ...
    return map; // already built, skip lines parsing below
  }
...
function analyze(words, rankMap) {
  const byRank = {};
...
async function downloadWikipedia(size) {
...
function loadWordList() {
  ...
  if (existsSync(gapFile)) {
    ...
    const words = JSON.parse(readFileSync(gapFile, "utf8"));
    const lemmaSet = buildDbLemmaSet();
    const missing = words.filter(w => !lemmaSet.has(w) && !lemmaSet.has(w.toLowerCase()));
    console.log(`Gap list: ${missing.length}/${words.length} words missing from database`);
    return missing;
  }
...
function printTable(rows, words) {
  const COL = 26;
  const NUM = 12;
...
async function main() {
  ...
  const combCurrentMaps = [newsMap, osMap].filter(Boolean);
  const combNewMaps     = [newsMap, wikiMap, subtlexMap, osMap].filter(Boolean);
  const combAllMaps     = loaded.map(c => c.map);
