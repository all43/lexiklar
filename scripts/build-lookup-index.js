...
const fileSize = fs.statSync(RAW_PATH).size;
...
const commitBatch = db.transaction((rows) => {
...
grep.stderr.on("data", (d) => process.stderr.write(d));
...
rl.on("line", (line) => {
  ...
  const offset    = parseInt(line.slice(0, colon), 10);
  const wordStart = colon + 11; // skip ':{"word": "' (11 chars)
...
rl.on("close", () => {
  ...
  const mb      = (fs.statSync(INDEX_PATH).size / 1e6).toFixed(1);
