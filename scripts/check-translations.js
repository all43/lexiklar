...
let filesToCheck = null; // null = all files
...
for (const posDir of readdirSync(WORDS_DIR).sort()) {
  ...
  const posStats = { files: 0, nullEn: 0, nullFull: 0, nullEnFiles: [], nullFullFiles: [] };
  ...
  for (const file of files) {
    ...
    let data;
    try { data = JSON.parse(readFileSync(join(dirPath, file), "utf-8")); } catch { continue; }
    posStats.files++;
