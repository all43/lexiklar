#!/usr/bin/env node
/**
 * fix-gloss-hints.ts — Validate and fix gloss_hint values in examples.json.
 *
...
import type { ExampleMap, Annotation } from "../types/example.js";
...
const BATCH_SIZE = batchSizeArgIdx !== -1 ? parseInt(args[batchSizeArgIdx + 1], 10) || 20 : 20;
const { provider: PROVIDER, model: MODEL_OVERRIDE } = parseProviderArgs(args, "lm-studio");


const WORDS_DIR = "data/words";
const POS_DIRS: Record<string, string> = { noun: "nouns", verb: "verbs", adjective: "adjectives" };
...
const wordCache = new Map<string, WordSenseData | null>();

function loadWordData(lemma: string, pos: string): WordSenseData | null {
  const key = `${lemma}|${pos}`;
  ...
  let data: WordSenseData | null = null;

  // Try exact match first
  const exactPath = join(dirPath, `${lemma}.json`);
  if (existsSync(exactPath)) {
    try { data = JSON.parse(readFileSync(exactPath, "utf-8")) as WordSenseData; } catch { /* skip */ }
  }
  ...
  if (!data) {
    try {
      ...
      if (files.length > 0) {
        ...
        const allSenses: Sense[] = [];
        for (const f of files) {
          try {
            const d = JSON.parse(readFileSync(join(dirPath, f), "utf-8")) as WordSenseData;
...
function getGlossesEn(data: WordSenseData | null): string[] {
  if (!data?.senses) return [];
  return data.senses.map((s) => s.gloss_en).filter((g): g is string => g !== null && g !== undefined);
}

function hintMatches(hint: string, glosses: string[]): boolean {
...
interface AutoFix {
  exId: string;
  annoIdx: number;
  reason: "no_file" | "single_sense";
}

interface LLMNeededItem {
  exId: string;
  annoIdx: number;
  text: string;
  lemma: string;
...
for (const [id, ex] of Object.entries(examples)) {
  ...
  for (let i = 0; i < ex.annotations.length; i++) {
    const a: Annotation = ex.annotations[i];
...
for (let b = 0; b < batches.length; b++) {
  ...
  try {
    ...
    const parsed: unknown = extractJSON(response.content);
    const rawItems = (typeof parsed === "object" && parsed !== null && "items" in parsed)
      ? (parsed as { items: unknown }).items
      : parsed;
    const items: HintResult[] = Array.isArray(rawItems) ? rawItems as HintResult[] : [];
    ...
    console.error(`  Batch ${b + 1} failed: ${message}`);
