 ...
 *
 * Usage:
 *   node scripts/translate-glosses.ts                         # default: OpenAI GPT-4.1-mini
 *   node scripts/translate-glosses.ts --provider anthropic    # Claude Haiku 4.5
 *   node scripts/translate-glosses.ts --provider ollama       # local Ollama (free, offline)
 *   node scripts/translate-glosses.ts --provider lm-studio   # local LM Studio
 *   node scripts/translate-glosses.ts --model gemma3:4b       # custom model (with any provider)
 *   node scripts/translate-glosses.ts --dry-run               # preview without API calls
 *   node scripts/translate-glosses.ts --concurrency 5        # run 5 API calls in parallel
 *   node scripts/translate-glosses.ts --reset --provider ...        # clear all gloss_en, then re-translate
 *   node scripts/translate-glosses.ts --reset-idioms --provider ... # clear gloss_en only for idiom phrases, then re-translate
 *
...
interface SenseToTranslate {
  senseIdx: number;
  gloss: string;
}

interface AlreadyDoneSense {
  senseIdx: number;
  gloss_en: string;
}
...
interface WordJob {
  ...
  toTranslate: SenseToTranslate[];
  alreadyDone: AlreadyDoneSense[];
}

/** Fields needed by buildUserPrompt — no file path required. */
export interface GlossPromptItem {
  senseIdx: number;
  word: string;
  pos: string;
  phraseType: string | null;
  gloss: string;
  allSenses: { gloss?: string | null; gloss_en?: string | null }[];
}

/** Full item including file path — used for write-back operations. */
interface SingleSenseItem extends GlossPromptItem {
  filePath: string;
  allSenses: Sense[];
}
 ...
 */
function collectWordBatches(targetField: "gloss_en" | "gloss_en_full"): WordJob[] {
  const jobs: WordJob[] = [];
  ...
  for (const posDir of POS_DIRS) {
    ...
    for (const file of readdirSync(dir)) {
      ...
      const toTranslate: SenseToTranslate[] = [];
      const alreadyDone: AlreadyDoneSense[] = [];
      for (let i = 0; i < (data.senses || []).length; i++) {
        ...
        const fieldValue = s[targetField];
        if (!fieldValue && s.gloss && !isArtifact) {
          toTranslate.push({ senseIdx: i, gloss: s.gloss });
        } else if (fieldValue) {
          alreadyDone.push({ senseIdx: i, gloss_en: fieldValue });
        }
      }
      if (toTranslate.length > 0) {
        const phraseType = (data as unknown as Record<string, unknown>).phrase_type as string | null ?? null;
        jobs.push({
          filePath,
          word: data.word,
          pos: data.pos,
...
function collectSenses(targetField: "gloss_en" | "gloss_en_full"): SingleSenseItem[] {
  const items: SingleSenseItem[] = [];
  for (const posDir of POS_DIRS) {
    const dir = join(WORDS_DIR, posDir);
    if (!existsSync(dir)) continue;
    for (const file of readdirSync(dir)) {
      ...
      for (let i = 0; i < (data.senses || []).length; i++) {
        ...
        if (!sense[targetField] && sense.gloss) {
          ...
          items.push({ filePath, senseIdx: i, word: data.word, pos: data.pos,
            phraseType, gloss: sense.gloss, allSenses: data.senses });
        }
      }
    }
  }
...
export function buildUserPrompt(item: GlossPromptItem): string {
 ...
 */
export function buildSingleSenseBatchPrompt(jobs: WordJob[]): string {
  return jobs
    .map((job, i) => {
 ...
 */
export function buildMultiSensePrompt(job: WordJob): string {
  const typeClause = job.phraseType ? `, phrase_type="${job.phraseType}"` : "";
 ...
 */
export function buildFullBatchPrompt(job: WordJob): string {
  const typeClause = job.phraseType ? `, phrase_type="${job.phraseType}"` : "";
...
const FULL_BATCH_SCHEMA: JsonSchemaObject = {
  type: "object",
  properties: {
    definitions: {
      type: "array",
...
function parseFullBatchResponse(content: string, expectedCount: number): string[] {
  const obj = extractJSON(content) as Record<string, unknown> | string[];
  const arr: unknown[] = Array.isArray(obj) ? obj : (obj as Record<string, unknown>).definitions as unknown[];
...
function parseBatchResponse(content: string, expectedCount: number): string[] {
  const obj = extractJSON(content) as Record<string, unknown> | string[];
  const arr: unknown[] = Array.isArray(obj) ? obj : (obj as Record<string, unknown>).translations as unknown[];
  if (!Array.isArray(arr)) throw new Error(`Expected array in response, got: ${JSON.stringify(obj)}`);
  if (arr.length < expectedCount) {
...
function writeTranslation(item: SingleSenseItem, translation: string): void {
  const data = JSON.parse(readFileSync(item.filePath, "utf-8")) as Record<string, unknown>;
  const senses = data.senses as Record<string, unknown>[];
  if (senses[item.senseIdx]) {
    const targetField = FULL_MODE ? "gloss_en_full" : "gloss_en";
    senses[item.senseIdx][targetField] = translation;
    senses[item.senseIdx][targetField + "_model"] = MODEL_LABEL;
  }
...
function writeWordTranslations(job: WordJob, targetField: string, translations: string[]): void {
  const data = JSON.parse(readFileSync(job.filePath, "utf-8")) as Record<string, unknown>;
  const senses = data.senses as Record<string, unknown>[];
  for (let i = 0; i < job.toTranslate.length; i++) {
    const { senseIdx } = job.toTranslate[i];
    if (senses[senseIdx]) {
      senses[senseIdx][targetField] = translations[i];
      senses[senseIdx][targetField + "_model"] = MODEL_LABEL;
    }
  }
  writeFileSync(job.filePath, JSON.stringify(data, null, 2) + "\n");
}

function printSummary(translated: number, errors: number, startAll: number, totalInputTokens: number, totalOutputTokens: number): void {
...
async function main(): Promise<void> {
  const targetField: "gloss_en" | "gloss_en_full" = FULL_MODE ? "gloss_en_full" : "gloss_en";
  ...
  if (RESET_GPT_MULTI && !DRY_RUN) {
    ...
    for (const posDir of POS_DIRS) {
      ...
      for (const file of readdirSync(dir)) {
        ...
        const senses = (data.senses as Record<string, unknown>[]) || [];
        if (senses.length <= 1) continue; // single-sense words: leave as-is
        let changed = false;
  ...
  if ((RESET || RESET_IDIOMS) && !DRY_RUN) {
    ...
    for (const posDir of POS_DIRS) {
      ...
      for (const file of readdirSync(dir)) {
        ...
        let changed = false;
        for (const sense of (data.senses as Record<string, unknown>[]) || []) {
  ...
  if (FULL_MODE) {
    ...
    await runPool(limitedJobs, CONCURRENCY, async (job, _i) => {
      ...
      try {
        const response: LLMResponse = await retryWithBackoff(
          () => callLLM(sysPrompt, userPrompt, {
            provider: PROVIDER, model: MODEL ?? undefined, maxTokens, temperature: 0.2,
        ...
        console.error(`  FAILED "${job.word}": ${(err as Error).message}`);
  ...
  const llmOptions = { provider: PROVIDER, model: MODEL ?? undefined, maxTokens: 128, temperature: 0.2 };
  // Batch call options: enforce structured output via JSON schema (OpenAI) or tool use (Anthropic).
  const batchOptions = {
    provider: PROVIDER, model: MODEL ?? undefined, temperature: 0.2,
    jsonSchema: TRANSLATIONS_SCHEMA as unknown as Record<string, unknown>,
  };

  // ── 1. Phrases (individual, PHRASE_SYSTEM_PROMPT) ────────────────────────
  await runPool(phraseJobs, CONCURRENCY, async (job) => {
    const sense = job.toTranslate[0];
    const item: SingleSenseItem = {
    ...
    try {
      ...
      console.error(`  FAILED phrase "${job.word}": ${(err as Error).message}`);
      errors++; callsDone++;
    }
  });
  ...
  await runPool(singleBatches, CONCURRENCY, async (batch, bi) => {
    ...
    try {
      ...
      console.error(`  FAILED batch ${bi + 1}: ${(err as Error).message}`);
