...
async function extractLeipzigWords(tarFile: string, destFile: string, label: string): Promise<void> {
...
interface PluralPreferredFile {
  words: string[];
}

function loadPluralPreferred(): Set<string> {
  if (!existsSync(PLURAL_PREFERRED_FILE)) return new Set();
  return new Set((JSON.parse(readFileSync(PLURAL_PREFERRED_FILE, "utf-8")) as PluralPreferredFile).words);
}
...
function enrichFiles(newsMap: FPMMap, wikiMap: FPMMap, subtlexMap: FPMMap, opensubMap: FPMMap): void {
  ...
  for (const dir of POS_DIRS) {
    ...
    for (const file of readdirSync(fullDir)) {
      ...
      const zipfNews    = toZipf(lookupFPM(newsMap, word));
      const zipfWiki    = toZipf(lookupFPM(wikiMap, word));
      const zipfSubtlex = toZipf(lookupFPM(subtlexMap, word));
      const zipfOsub    = toZipf(lookupFPM(opensubMap, word));
...
function printCheckTable(newsMap: FPMMap, wikiMap: FPMMap, subtlexMap: FPMMap, opensubMap: FPMMap): void {
  const col = (v: number | null, w: number = 7): string =>
