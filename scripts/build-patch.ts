...
function main(): void {
  ...
  // Read versions
  const oldVersion = (oldDb.prepare("SELECT value FROM meta WHERE key = 'version'").get() as MetaRow | undefined)?.value;
  const newVersion = (newDb.prepare("SELECT value FROM meta WHERE key = 'version'").get() as MetaRow | undefined)?.value;
  const newBuiltAt = (newDb.prepare("SELECT value FROM meta WHERE key = 'built_at'").get() as MetaRow | undefined)?.value;
  ...
  for (const row of oldDb.prepare("SELECT id, file, hash FROM words").all() as OldWordRow[]) {
    oldWords.set(row.file, { id: row.id, hash: row.hash });
  }

  const newWords = new Map<string, NewWordRow>();
  for (const row of newDb.prepare("SELECT id, file, hash, lemma, lemma_folded, pos, gender, frequency, plural_dominant, plural_form, gloss_en, data FROM words").all() as NewWordRow[]) {
    newWords.set(row.file, row);
  }

  const changedWordFiles: string[] = [];
  const newWordFiles: string[] = [];
  let deletedWords = 0;
  ...
  for (const row of oldDb.prepare("SELECT id, hash FROM examples").all() as OldExampleRow[]) {
    oldExamples.set(row.id, row.hash);
  }

  const newExamples = new Map<string, NewExampleRow>();
  for (const row of newDb.prepare("SELECT id, hash, data FROM examples").all() as NewExampleRow[]) {
  ...
  let manifest: PatchManifest = { current_version: newVersion, built_at: newBuiltAt, patches: {}, full_db: {} };

  if (existsSync(manifestPath)) {
    try {
      manifest = JSON.parse(readFileSync(manifestPath, "utf-8")) as PatchManifest;
    } catch {
      // Start fresh
    }
  }
