...
function heuristicCheck(word: string, gloss_en: string): string[] {
...
interface HeuristicFlagged extends IdiomEntry {
  reasons: string[];
}

interface LLMFlagged {
  idx?: number;
  word: string;
  current?: string;
  issue?: string;
...
interface BatchItem extends IdiomEntry {
  idx: number;
}

// ── Load idioms ───────────────────────────────────────────────────────────────

function loadTranslatedIdioms(): IdiomEntry[] {
  const idioms: IdiomEntry[] = [];
  for (const file of readdirSync(PHRASES_DIR)) {
    if (!file.endsWith(".json")) continue;
    let data: PhraseData;
    try { data = JSON.parse(readFileSync(join(PHRASES_DIR, file), "utf-8")) as PhraseData; }
    catch { continue; }
...
function buildBatchPrompt(batch: BatchItem[]): string {
  return batch
    .map(({ idx, word, gloss_en, gloss }) =>
      `${idx}. "${word}" → "${gloss_en}"\n   (German meaning: ${gloss.slice(0, 80)})`,
    )
    .join("\n");
}

async function llmReview(idioms: IdiomEntry[]): Promise<LLMFlagged[]> {
...
interface PhraseFileData {
  senses?: Array<{ gloss_en?: string | null; [key: string]: unknown }>;
  [key: string]: unknown;
}
 ...
 */
function applyFixes(flagged: LLMFlagged[], wordToFile: Map<string, string>): void {
  let applied = 0;
  let skipped = 0;
  ...
  for (const { word, suggestion } of flagged) {
    ...
    let data: PhraseFileData;
    try { data = JSON.parse(readFileSync(filePath, "utf-8")) as PhraseFileData; }
...
function printHeuristic(flagged: HeuristicFlagged[]): void {
...
function printLLM(flagged: LLMFlagged[]): void {
  if (JSON_OUT) return;
  console.log(`\n── LLM review ──────────────────────────────────────────────────────────`);
...
const wordToFile = new Map<string, string>(allIdioms.map(({ word, file }) => [word, join(PHRASES_DIR, file)]));

// Phase 1: heuristics
const heuristicFlagged: HeuristicFlagged[] = idioms
