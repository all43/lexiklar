...
const verbs = [];
const verbsByWord = new Map(); // word → [verb, ...]

for (const file of readdirSync(VERBS_DIR).sort()) {
  if (!file.endsWith(".json")) continue;
  try {
    const data = JSON.parse(readFileSync(join(VERBS_DIR, file), "utf-8"));
    data._file = file.replace(".json", "");
    verbs.push(data);
    if (!verbsByWord.has(data.word)) verbsByWord.set(data.word, []);
    verbsByWord.get(data.word).push(data);
  } catch {
    // skip unreadable
  }
}
...
function report(check, verb, detail, extra = {}) {
...
for (const verb of verbs) {
  ...
  for (const tense of TENSES) {
    for (const [person, form] of Object.entries(conj[tense] || {})) {
      if (!form) continue;
...
const CORE_STEM_KEYS = ["past", "subj2", "present_du_er"];
...
const EXPECTED_PERSONS = ["ich", "du", "er", "wir", "ihr", "sie"];
...
for (const verb of verbs) {
  ...
  for (const tense of ["present", "preterite"]) {
    const tenseObj = conj[tense] || {};
...
for (const [check, list] of Object.entries(byCheck)) {
  console.log(`\n── ${CHECK_DESCRIPTIONS[check] || check} (${list.length}) ──`);
